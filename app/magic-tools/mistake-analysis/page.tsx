...
import { Card, Button, Tabs, Chart } from '../../components/ui';
...
export default function MistakeAnalysisPage() {
  ...
  const generatePractice = (mistake: Mistake) => {
    router.push(`/magic-tools/practice-generator?topic=${mistake.topic}&subject=${mistake.subject}`);
  };

  const reviewMistake = (mistake: Mistake) => {
    router.push(`/magic-tools/ai-tutor?mode=homework&question=${mistake.id}`);
  };
        ...
        <div className={styles.practiceView}>
          <Card className={styles.practiceCard}>
            ...
            <div className={styles.planActions}>
              <Button variant="primary" onClick={() => router.push('/magic-tools/practice-generator')}>
                Start Practice Session 开始练习
              </Button>
              <Button variant="secondary" onClick={() => router.push('/magic-tools/ai-tutor')}>
                Get AI Help 获取AI帮助
              </Button>
            </div>
          </Card>
